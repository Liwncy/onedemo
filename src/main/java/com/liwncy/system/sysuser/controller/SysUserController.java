package com.liwncy.system.sysuser.controller;
...
import com.liwncy.common.utils.PinyinUtils;
...
public class SysUserController {
    ...
    public DataGridView loadAllUser(UserVo userVo){
        ...
        for (SysUser user : list) {
            ...
            if (deptid!=null){
                ...
                SysDept one = deptService.getById(deptid);
                //设置user的部门名称
                user.setDeptname(one.getName());
            }
            Integer mgr = user.getMgr();
            if (mgr!=null){
                SysUser one = userService.getById(mgr);
                //设置user的领导名称
    ...
    public Map<String,Object> loadUserMaxOrderNum(){
        ...
        List<SysUser> list = userService.page(page,queryWrapper).getRecords();
        if (list.size()>0){
    ...
    public ResultObj changePassword(String oldPassword,String newPwdOne,String newPwdTwo){
        ...
        SysUser user1 = userService.getById(userId);
        //2.1获得该用户的盐
